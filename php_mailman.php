...
class php_mailman
{
	...
	public function __construct($domain='', $adminpasswd='', $listname='', $protocol='', $listlanguage='', $notifyowner='', $notifyuser='', $digest='') {
		...
		if($adminpasswd != '') $this->adminpasswd = $adminpasswd;
		
		if($listname != '') $this->listname = $listname;
		
		if($protocol != '') $this->protocol = $protocol;
		
		if($listlanguage != '') $this->listlanguage = $listlanguage;
		
		if($notifyowner != '') $this->notifyowner = $notifyowner;
	...
	public function subscribe($email='') {
		if($email!='') return $this->api( $this->prepare_url(self::URL_SUBSCRIBE, $email) );
		else return false;
	}
	...
	public function unsubscribe($email='') {
		if($email!='') return $this->api( $this->prepare_url(self::URL_UNSUBSCRIBE, $email) );
		else return false;
	}
	...
	public function digest($email='') {
		if($email!='') return $this->api( $this->prepare_url(self::URL_SET_DIGEST, $email) );
		else return false;
	}
	...
	public function list_member($email='') {
		if($email!='') return $this->api( $this->prepare_url(self::URL_LIST_A_MEMBER, $email) );
		else return false;
	}
